...
- ✅ **定时重启** - 按固定间隔（小时）或每天指定时刻自动重启 frpc / frps，定期刷新连接、回收资源
...
- **定时重启 FRP（frpc / frps）**：「系统设置 → 守护与告警」新增「定时重启」区块，支持两种触发方式——**按间隔**（每 N 小时，1–720）或**每天固定时刻**（HH:MM）；frpc / frps 可分别勾选是否参与。仅重启「正在运行或已登记自愈」的进程，不会擅自拉起未运行的 frp；重启失败会写 `[面板]` 事件日志并触发告警推送。设置存 `configs/app_settings.ini` 的 `[restart]` 段，监控线程每 5 秒检查一次，修改间隔自动重新计时、关闭期间不累积。新增 `GET/POST /api/settings/autorestart`，区块右上角实时显示「下次触发时间 / 各进程上次重启时间」。
- **README 双语化**：新增英文版 `README_EN.md`，中英版本顶部可一键互切。
- **感谢区新增打赏二维码**：如果这个项目帮到了你，欢迎请作者喝杯咖啡 ☕。
...
- **翻译机制升级（主站 + 登录页）**
  - 多行文本节点逐行翻译：HTML 的换行缩进会把整段变成一个节点，此前整块查表必然失败，长篇说明因此残留中文
  - 新增「抹空白」规范化索引，源码缩进不再影响查表命中
  - 新增 `MutationObserver` 兜底：渲染晚于翻译的动态内容（状态轮询、列表刷新、`title` 悬浮提示）自动补翻
  - 切回中文精确还原：记录中英双向映射，逐节点回放
- **修复切换后仍有残留中文**：`toggleLang()` 改为「先写服务端语言偏好 → 再重绘动态内容」；服务端写入后立刻作废 `web_ui / web_auth / frp_manager` 三处语言缓存——此前重绘时后端仍按旧语言返回文案
- **后端文案双语化**：代理映射（内网 / 外网 / 域名）、服务端端口标签（FRP 服务端口 / 管理面板 / 允许端口范围）、认证方式、管理 API 端点说明、网络故障人话翻译（拒绝连接 / 超时 / 不可达）、HTTP 链接探测日志、安全扫描日志与风险项、定时重启日志；`frp_manager.py` 新增 `_m()` 语言助手
- **按钮配色**：统一科技感按钮体系（主操作青 / 靛渐变 + 霓虹辉光，次级玻璃幽灵按钮 + 青色悬停辉光）
- **登录页版本号修正**：右上角版本号此前硬编码为 `v1.1`，改为跟随实际版本（`web_auth` 新增懒加载的 `_app_version()`，避免与 `web_ui` 循环导入）
- **验证**：`py_compile` ×5、内联 script `node --check`、HTML 标签平衡、Flask `test_client` e2e、接线与 CSS 变量自检全通过；并用无头浏览器对登录页 + 5 个主页面做了「中→英」与「英→中」往返核验，英文态可见中文残留仅剩语言按钮自身的目标语言字样

### v1.13.1
- **修复语言切换按钮无效**：🌐 按钮此前未绑定点击事件，点击无反应；现已修复，并在启动时恢复上次选择的语言（localStorage + 服务器持久化双通道）
- **补全界面翻译覆盖**：侧栏一级页面（运行状态/配置与下载/系统设置/安全中心/运行日志）、顶栏页名、各区块标题（运行状态、进程自愈守护、端口映射、服务端连接信息、frp 版本控制、Web 设置、守护与告警、配置编辑器、配置生成器、配置安全扫描、运行日志等）全部纳入双语

### v1.13.0
- **定时重启 FRP（frpc / frps）**：「系统设置 → 守护与告警 → 定时重启」，支持按间隔（每 N 小时）或每天固定时刻自动重启，frpc / frps 分别勾选；只重启运行中/已登记自愈的进程，失败写事件日志并推送告警；设置存 `configs/app_settings.ini [restart]`，新增 `GET/POST /api/settings/autorestart`
- **README 双语化**：新增英文版 `README_EN.md`，顶部一键互切
- **感谢区新增打赏二维码**
...
如果这个项目帮到了你，欢迎请作者喝杯咖啡 —— **感谢您的打赏，是我持续开发的动力！**

<div align="center">

<img src="docs/donate-wechat.jpg" alt="微信打赏收款码" width="280">
